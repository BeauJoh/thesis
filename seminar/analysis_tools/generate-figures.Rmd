...
```{r, load_libraries, include=FALSE, cache=FALSE, echo=FALSE}
...
```{r, load_aiwc_data, include=FALSE, cache=FALSE, echo=FALSE}

load('../analysis_tools/newfeat.Rdf')

#source('../analysis_tools/load_aiwc_dataframes.R')
source('../analysis_tools/restructure_old_aiwc_data.R')
...
```{r eval=TRUE, dev='pdf', echo=FALSE, cache=FALSE,include=FALSE, generate_lud_perimeter_all_kiviat}
# TODO: load new aiwc results with load imbalance as newfeatdata.lud
source('../analysis_tools/restructure_aiwc_data.R')
rm(featdata.lud)
source('../analysis_tools/load_aiwc_lud_updated_dataframes.R')
newfeatdata.lud <- featdata.lud
newfeatdata.lud <- drop_metrics_for_simple_kiviat(newfeatdata.lud)
x <- reorder_and_subset(featdata=newfeatdata.lud,size='tiny',kernel="lud_perimeter")
x <- data.frame(t(colMeans(x)))
y <- x
x <- reorder_and_subset(featdata=newfeatdata.lud,size='small',kernel="lud_perimeter")
x <- data.frame(t(colMeans(x)))
y <- rbind(y,x)
x <- reorder_and_subset(featdata=newfeatdata.lud,size='medium',kernel="lud_perimeter")
x <- data.frame(t(colMeans(x)))
y <- rbind(y,x)
x <- reorder_and_subset(featdata=newfeatdata.lud,size='large',kernel="lud_perimeter")
x <- data.frame(t(colMeans(x)))
y <- rbind(y,x)
#TODO: newfeatdata.lud should be featdata.all for normalization vector -- instead this just removes the uncomputed load_imbalance results, since only the lud application has been updated with new awic metrics to measure load imbalance
featdata.all <- subset(featdata.all, application!="lud")
featdata.all$metric <- as.character(featdata.all$metric)
featdata.all$metric[featdata.all$metric == "max instructions to barrier"] <- "max instructions executed by a work-item"
featdata.all$metric[featdata.all$metric == "min instructions to barrier"] <- "min instructions executed by a work-item"
featdata.all$metric <- as.factor(featdata.all$metric)
featdata.all <- rbind(featdata.all,featdata.lud)
nv <- new_reorder_features(featdata.all)
nv$load_imbalance[which(nv$application!="lud")] <- 0
nv <- subset(nv,select=-c(size,kernel,application,invocation))
plot_kiviat(y,nv,colour="black",colour_spokes=TRUE,all_sizes=TRUE)
z1 <- recordPlot()

x <- reorder_and_subset(featdata=newfeatdata.lud,size='tiny',kernel="lud_internal")
x <- data.frame(t(colMeans(x)))
y <- x
x <- reorder_and_subset(featdata=newfeatdata.lud,size='small',kernel="lud_internal")
x <- data.frame(t(colMeans(x)))
y <- rbind(y,x)
x <- reorder_and_subset(featdata=newfeatdata.lud,size='medium',kernel="lud_internal")
x <- data.frame(t(colMeans(x)))
y <- rbind(y,x)
x <- reorder_and_subset(featdata=newfeatdata.lud,size='large',kernel="lud_internal")
x <- data.frame(t(colMeans(x)))
y <- rbind(y,x)
...
x <- reorder_and_subset(featdata=newfeatdata.lud,size='tiny',kernel="lud_diagonal")
x <- data.frame(t(colMeans(x)))
y <- x
x <- reorder_and_subset(featdata=newfeatdata.lud,size='small',kernel="lud_diagonal")
x <- data.frame(t(colMeans(x)))
y <- rbind(y,x)
x <- reorder_and_subset(featdata=newfeatdata.lud,size='medium',kernel="lud_diagonal")
x <- data.frame(t(colMeans(x)))
y <- rbind(y,x)
x <- reorder_and_subset(featdata=newfeatdata.lud,size='large',kernel="lud_diagonal")
x <- data.frame(t(colMeans(x)))
y <- rbind(y,x)
...
source('../analysis_tools/generate_simple_lud_lines.R')

```

```{r, build_stacked_plots, include=FALSE, cache=TRUE, echo=FALSE}
source('../analysis_tools/stacked_features.R')
```


```{r eval=TRUE, dev='pdf', cache=TRUE, echo=FALSE, include=FALSE, draw_stacked_plots, fig.cap = "\\label{fig:stacked_plots} Selected AIWC metrics from each category over all kernels and 4 problem sizes.", fig.width=11.7,fig.height=8.3}
...
```{r eval=TRUE, dev='pdf', echo=FALSE, include=FALSE, cache=TRUE, draw_lud_diagonal_internal_all_kiviat, fig.cap = "\\label{fig:lud_diagonal_internal_kiviat} feature-space of the \\texttt{diagonal} and \\texttt{internal} kernels in the lud application on all problem sizes.", fig.width=16,fig.height=8}
print(plot_grid(z2,z3,labels=c('A) LUD Diagonal','B) LUD Internal'),nrow=1))#labels = "AUTO"
```

```{r eval=TRUE, dev='pdf', cache=TRUE, echo=FALSE, include=FALSE, draw_lud_diagonal_perimeter_lmae_all_kiviat, fig.cap = "\\label{fig:lud_perimeter_kiviat_and_lmae} feature-space of the \\texttt{perimeter} kernel in the lud application on all problem sizes and Local Memory Address Entropy over the tiny problem size.", fig.width=16,fig.height=8}
print(plot_grid(z1,df_plot,labels=c('       A) LUD Perimeter','B) LUD Perimeter LMAE on tiny'),nrow=1))#labels = "AUTO"
```

```{r eval=TRUE, dev='pdf', echo=FALSE, include=FALSE, cache=TRUE, draw_lud_diagonal_all_kiviat, fig.width=8,fig.height=8}
print(plot_grid(z2,labels=c('LUD Diagonal'),nrow=1))#labels = "AUTO"
```

```{r eval=TRUE, dev='pdf', echo=FALSE, include=FALSE, cache=TRUE, draw_lud_internal_all_kiviat, fig.width=8,fig.height=8}
print(plot_grid(z3,labels=c('LUD Internal'),nrow=1))#labels = "AUTO"
```

```{r eval=TRUE, dev='pdf', echo=FALSE, include=FALSE, cache=TRUE, draw_lud_perimeter_all_kiviat, fig.width=8,fig.height=8}
print(plot_grid(z1,labels=c('LUD Perimeter'),nrow=1))#labels = "AUTO"
```

```{r eval=TRUE, dev='pdf', echo=FALSE, include=FALSE, cache=TRUE, draw_lud_perimeter_lmae_tiny_kiviat, fig.width=8,fig.height=8}
print(plot_grid(df_plot,labels=c('LUD Perimeter LMAE on tiny'),nrow=1))#labels = "AUTO"
```
