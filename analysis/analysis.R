...
p <- ggplot(energy_df,aes(x=year,y=value,colour=metric)) + geom_line() + expand_limits(y = 0) + labs(x="Publication Year (TOP500 June issue)",y=TeX("Power Efficiency $\\left[$mean$\\left(\\frac{$GFlops$}{$Watt$}\\right)\\right]$"),colour="Metric") + scale_color_discrete(labels=c("supercomputers with accelerators",'supercomputers without accelerators')) #scale_colour_manual(labels=c("Machines using Accelerators in the TOP500",TeX("mean($\\frac{Accelerator Cores}{Total Cores}$)")),values=c("blue","red"))#title="Accelerator use in the Top500",
